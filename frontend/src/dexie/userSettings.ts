 ...
 */
export async function loadUserSetting(key: string): Promise<string | number | boolean | undefined> {
  // Check if the key is valid
  if (!isValidSettingKey(key)) {
    throw new Error(`Invalid setting key: ${key}`);
  }
  ...
  if (!setting) {
    ...
    await saveUserSetting(key, defaultValue);
    return defaultValue;
  }
...
export async function loadAllUserSettings(): Promise<UserSettings> {
  ...
  const existingSettingsMap = new Map<string, string | number | boolean>();
  ...
  for (const key of allSettingKeys) {
    if (existingSettingsMap.has(key)) {
      ...
      if (validateSettingValue(key, value)) {
        ...
        saveUserSetting(key, defaultValue).catch(error => {
