...
export function DeckCheckDialog({
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-h-[calc(100dvh-3rem)] max-w-[min(800px,calc(100vw-2rem))] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Deck check</DialogTitle>
          <DialogDescription>
            Review Crossfire practice support and deck structure before starting a game.
          </DialogDescription>
        </DialogHeader>
                  ...
                  {group.issues.map(issue => (
                    ...
                    >
                      <span className="min-w-0 break-words">
                        {issue.cardId
                          ? deckCardName(catalog, issue.cardId)
                          : words(issue.zone ?? '')}
                      </span>
