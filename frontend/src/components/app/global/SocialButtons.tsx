...
const SocialButtons: React.FC<SocialButtonsProps> = ({ location }) => {
  const isHeader = location === 'header';
  const containerClass = isHeader
    ? 'hidden md:flex flex-col items-center gap-2 py-2'
    : 'flex md:hidden justify-center md:opacity-80 items-center gap-2';

  const discordClass = isHeader
  ...
  return (
    <div className={containerClass}>
      <a href={DISCORD_LINK} target="_blank" rel="noopener noreferrer" className={discordClass}>
        <svg
          className="w-4 h-4"
          viewBox="0 0 24 24"
          fill="currentColor"
