...
import { useGetDecksForModals } from '@/api/decks/useGetDecksForModals.ts';
import { CardStatsParams } from '@/api/card-stats';

type DecksForModalProps = CardStatsParams;
...
const DecksForModal: React.FC<DecksForModalProps> = ({
  metaId,
  tournamentId,
  tournamentGroupId,
  leaderCardId,
  ...
  const { data, isLoading, error } = useGetDecksForModals({
    metaId,
    tournamentId,
    tournamentGroupId,
    leaderCardId,
  ...
  // Map unique decks and gather related deckCards when available (may be empty here)
