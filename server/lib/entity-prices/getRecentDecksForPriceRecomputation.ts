 ...
 * - entity_price.updated_at < deck.updated_at (deck changed in the meantime), OR
 * - entity_price.updated_at < 8 hours ago (new prices could be loaded (not always the case, but for now, 8h should be ok))
    ...
    .where(
      and(
        ...
        or(
          ...
          lt(entityPrice.updatedAt, deck.updatedAt),
