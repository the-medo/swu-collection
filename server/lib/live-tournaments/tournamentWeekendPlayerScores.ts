  ...
  await db.execute(sql`
    WITH score_rows AS (
      SELECT
        ...
        CASE
          WHEN twm.player_id_2 IS NULL OR twm.player_1_game_win > twm.player_2_game_win THEN 1
          ELSE 0
        END AS match_wins,
        CASE
          WHEN twm.player_id_2 IS NOT NULL AND twm.player_1_game_win < twm.player_2_game_win THEN 1
          ELSE 0
        END AS match_losses,
        CASE
          WHEN twm.player_id_2 IS NOT NULL AND twm.player_1_game_win = twm.player_2_game_win THEN 1
      ...
      FROM tournament_weekend_player twp
        JOIN tournament_weekend_match twm
          ...
          AND twm.player_id_1 = twp.player_id
      WHERE
        twp.tournament_id = ${tournamentId}
        AND (
          twm.player_id_2 IS NULL
          OR (
      ...
      FROM tournament_weekend_player twp
        JOIN tournament_weekend_match twm
          ON twp.tournament_id = twm.tournament_id
          AND twm.player_id_2 = twp.player_id
      WHERE
        twp.tournament_id = ${tournamentId}
    ...
    FROM tournament_weekend_player twp
      LEFT JOIN scores
        ...
        AND scores.player_id = twp.player_id
