...
description: Change SWUBASE live tournament weekends, Melee polling/progress, final-import enqueueing, live-home caches/patches, streams/resources, or watched-player behavior.
---
...
the polling/enqueue/reconciliation jobs. Final tournament data import and its
worker belong to `swubase-tournament-imports`;
`server/lib/live-tournaments/tournamentImportQueue.ts` is a crossover file that
...
- A finished supported-format tournament with decklists may enqueue one import.
  The unique tournament import row prevents duplicate enqueueing. Queue claims,
  final data writes, derived work, and failure recovery belong to the tournament
  import skill.
...
jobs, `swubase-tournament-imports` for the final import worker,
`swubase-discord-notifications` for announcements, `swubase-screenshotter` for
import media, and the API/frontend skills for route or screen changes.
...
polling, duplicate enqueue prevention, cache version gaps, watched-player
scoping, and resource approval transitions. Run the frontend
build when shared contracts or live-home UI change. Database isolation alone is
not enough for polling tests: mock the domain calls or explicitly review
development-only Melee and notification configuration.
