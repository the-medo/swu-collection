...
| Task pattern | Primary skill | Companion skills |
| --- | --- | --- |
| Design/change Crossfire online games, authoritative engine state, commands, viewer projections, spectators, recovery, replays, scenarios, or dynamic card releases | `swubase-online-play` | `swubase-architecture` for structural decisions; deck/catalog, auth, API, WebSocket, frontend, migration, development-data, or worktree skills for boundaries actually crossed; `swubase-validation`, `swubase-change-review` for source/tooling changes |
| Implement/fix an individual card's Crossfire gameplay behavior, keyword interactions, or card conformance scenarios | `swubase-card-implementator` | `swubase-online-play` for engine/state/visibility/replay changes; `swubase-card-catalog` for identity resolution/changes; `swubase-validation`, `swubase-change-review` |
