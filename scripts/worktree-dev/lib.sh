...
readonly SWUBASE_WORKTREE_CROSSFIRE_PORT_START="3110"
readonly SWUBASE_WORKTREE_CROSSFIRE_PORT_END="3199"
...
write_local_state_contents() {
...

  {
    ...
    write_assignment SWUBASE_CROSSFIRE_PORT "${SWUBASE_CROSSFIRE_PORT}"
...
write_worktree_env_contents() {
...

  {
    ...
    write_assignment CROSSFIRE_HOST 127.0.0.1
    write_assignment CROSSFIRE_PORT "${SWUBASE_CROSSFIRE_PORT}"
    ...
    write_assignment CROSSFIRE_PROXY_URL "http://127.0.0.1:${SWUBASE_CROSSFIRE_PORT}"
    write_assignment VITE_CROSSFIRE_WS_URL "/api/ws/crossfire/:gameId"
...
create_current_state() {
  ...
  SWUBASE_CROSSFIRE_PORT=$(claim_port crossfire "${SWUBASE_WORKTREE_CROSSFIRE_PORT_START}" "${SWUBASE_WORKTREE_CROSSFIRE_PORT_END}")
  ...
  if load_current_state; then
    if [[ "${SWUBASE_WORKTREE_ID}" == "${EXPECTED_WORKTREE_ID}" ]]; then
      ...
      if [[ "${SWUBASE_WORKTREE_PUBLIC_ORIGIN}" != "${REQUESTED_SWUBASE_WORKTREE_PUBLIC_ORIGIN}" \
        ...
        if service_is_running backend || service_is_running frontend || service_is_running crossfire; then
      ...
      SWUBASE_WORKTREE_BRANCH="${EXPECTED_WORKTREE_BRANCH}"
      SWUBASE_WORKTREE_REVISION="${EXPECTED_WORKTREE_REVISION}"
      # Upgrade older local state without replacing its database or app ports.
      if [[ -z "${SWUBASE_CROSSFIRE_PORT:-}" ]]; then
        require_command ss
        SWUBASE_CROSSFIRE_PORT=$(claim_port crossfire "${SWUBASE_WORKTREE_CROSSFIRE_PORT_START}" "${SWUBASE_WORKTREE_CROSSFIRE_PORT_END}")
      fi
      write_current_state
...
crossfire_enabled() {
  local enabled="${CROSSFIRE_ENABLED-}"
  if [[ ! -v CROSSFIRE_ENABLED ]]; then
    enabled=$(dotenv_value "${SWUBASE_WORKTREE_REPOSITORY_DIR}/.env" CROSSFIRE_ENABLED || true)
  fi
  [[ "${enabled}" == 1 ]]
}
...
start_crossfire_service() {
  ...
  if ! crossfire_enabled; then
    stop_service crossfire
    return 0
  fi
  service_is_running crossfire && return 0
  port_is_in_use "${SWUBASE_CROSSFIRE_PORT}" \
    && fail "Crossfire port ${SWUBASE_CROSSFIRE_PORT} is occupied by an unmanaged process."
  pid_file=$(pid_file_for_service crossfire)
  log_file=$(log_file_for_service crossfire)
  rm -f -- "${pid_file}"
  log_info "Starting Crossfire on http://127.0.0.1:${SWUBASE_CROSSFIRE_PORT}."
  (
    cd -- "${SWUBASE_WORKTREE_REPOSITORY_DIR}"
    exec setsid nohup env \
      ENVIRONMENT=local \
      ...
      CROSSFIRE_PORT="${SWUBASE_CROSSFIRE_PORT}" \
      BETTER_AUTH_URL="${SWUBASE_WORKTREE_PUBLIC_ORIGIN}" \
      bun play/worker/index.ts
  ) >"${log_file}" 2>&1 &
  printf '%s\n' "$!" > "${pid_file}"
}
...
start_frontend_service() {
  ...
  (
    ...
    exec setsid nohup env \
      ...
      CROSSFIRE_PROXY_URL="http://127.0.0.1:${SWUBASE_CROSSFIRE_PORT}" \
      VITE_CROSSFIRE_WS_URL="/api/ws/crossfire/:gameId" \
...
print_status() {
  ...
  local crossfire_status="disabled"
  local crossfire_address="not allocated; run setup/up"
  ...
  crossfire_enabled && crossfire_status="stopped"
  service_is_running crossfire && crossfire_status="running"
  [[ -n "${SWUBASE_CROSSFIRE_PORT:-}" ]] && crossfire_address="http://127.0.0.1:${SWUBASE_CROSSFIRE_PORT}"
...
Crossfire: ${crossfire_address} (${crossfire_status})
...
print_logs() {
  ...
  case "${service_name}" in
    ...
    backend|frontend|crossfire)
      ...
      ;;
    all)
      for service_name in backend frontend crossfire; do
        local log_file
        log_file=$(log_file_for_service "${service_name}")
        if [[ -f "${log_file}" ]]; then
          printf '\n=== %s ===\n' "${service_name}"
      ...
      ;;
    *)
      fail "Unknown log target '${service_name}'. Use database, backend, frontend, crossfire, or all."
      ;;
  esac
}
